...
SPEEDCHECK_MAX_SOCKETS = 120        -- Maks socket TCP probe aktif bersamaan

-- Parallel Upstream Aggregation (auto-select from newServer() by latency)
-- Kosong = otomatis pilih dari newServer() yang paling cepat
PARALLEL_UPSTREAMS            = {}   -- Manual override: {"8.8.8.8", "1.1.1.1"}
PARALLEL_UPSTREAM_MAX_LATENCY = 300  -- Skip upstream jika latency > ms ini
PARALLEL_MAX_UPSTREAMS        = 5    -- Maks upstream yang dipakai sekaligus
PARALLEL_UPSTREAM_REFRESH_SEC = 30   -- Seberapa sering rebuild list (detik)
...
1. **First query** for `x.com` → DnsDist forwards the query to an upstream and returns the response immediately (no stalling). In the background, SmartDist schedules the domain for probing.
2. **Smart Upstream Selection** → Every 30s (configurable), SmartDist calls `getServers()` to get all `newServer()` entries, measures their current latency, and auto-selects the **top N fastest** that respond within `PARALLEL_UPSTREAM_MAX_LATENCY` ms. Slow or down servers are skipped.
3. **Parallel UDP Queries** → SmartDist sends DNS queries for the domain to those top N upstreams simultaneously, aggregating all returned IPs into one candidate pool.
4. **Background TCP Probing** (`maintenance()`, runs every ~1s) → Non-blocking TCP connects to all candidate IPs on ports 80 and 443. The first `SPEEDCHECK_MAX_RESPONSE` IPs to respond are cached as winners.
5. **Next query** for `x.com` → DnsDist rewrites the response. In `fastest-ip` mode: reorder the packet so the fastest IP is first. In `fastest-response` mode: overwrite all records with the top N fastest IPs (round-robin).
...
Set these in `dnsdist.conf` after all rules, before calling `smartdns_enable_speedcheck()`.
...
| `SPEEDCHECK_PORTS` | `{80, 443}` | Ports to probe |
| `SPEEDCHECK_MAX_IPS` | `6` | Max IPs probed per domain |
| `SPEEDCHECK_MAX_RESPONSE`| `2` | Max winning IPs to return (`fastest-response` mode) |
| `SPEEDCHECK_CACHE_TTL` | `300` | Seconds to cache fastest IP result |
| `SPEEDCHECK_QUEUE_LIMIT` | `200` | Max concurrent probe queue size |
| `SPEEDCHECK_MAX_SOCKETS` | `120` | Hard cap on active TCP probe sockets |

#### Parallel Upstream Aggregation

| Variable | Default | Description |
|---|---|---|
| `PARALLEL_UPSTREAMS` | `{}` (auto) | Manual upstream list. Empty = auto-select from `newServer()` |
| `PARALLEL_UPSTREAM_MAX_LATENCY` | `300` | Skip upstreams slower than this (ms) |
| `PARALLEL_MAX_UPSTREAMS` | `5` | Max upstreams used simultaneously |
| `PARALLEL_UPSTREAM_REFRESH_SEC` | `30` | How often to rebuild the active upstream list |
| `SPEEDCHECK_MAX_PAR_SOCKETS` | `30` | Hard cap on parallel UDP sockets |
